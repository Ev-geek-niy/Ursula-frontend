...
export class Player {
  ...
  @mx public hand: Array<Card>
  @mx public deck: Array<Card>
...
export class GameState {
  ...
  private currentMana: number = Constants.STARTING_MANA
  ...
  public executeCardEffect(playerID:string, cardID:number, target: Tile) {
    ...
    if(this.currentTurn === playerID) {
      ...
      if(sourcePlayer.player.mana >= Cards[cardID].cost) {
        ...
        sourcePlayer.player.hand.splice(sourcePlayer.player.hand.findIndex(card => cardID == card.id),1)
      }
  ...
  public changeTurn() {
    ...
    if(this.gameState === "game") {
      ...
      let player = this.players.get(this.currentTurn)
      let playerDeck = player.deck
      for(let i = 1; i <= 5 - player.hand.length; i++) {
          player.hand.push(this.getCardFromDeck(playerDeck))
      }
      ...
      if(this.currentTurn === this.playerA.ID) {
        ...
        // Игрок B закончил ход - увеличиваем ману на 1
        if(this.currentMana < Constants.MAX_MANA) {
          this.currentMana += 1
          this.players.forEach( player => {
            player.mana += 1
          })
  ...
  public getCardFromDeck(playerDeck: Array<Card>) {
  ...
  public FillDeck(playerDeck: Array<Card>) {
    Cards.forEach( card => {
